...
async function buildGame(name) {
  ...
  // Синтаксис проверяется ЗДЕСЬ, до записи файла. Сборщик склеивает текст и
  // сам по себе разбирать код не обязан — из-за этого сломанная скобка или
  // неэкранированный апостроф в строке уезжали в dist под видом успешной
  // сборки: «npm run build» печатал зелёные строки и выходил с нулём, а
  // страница у человека открывалась пустой. Ловил это только npm test, и то
  // по обвалу чужих тестов. new Function код не выполняет — только разбирает.
  for (const { rel, code } of chunks) {
    try { new Function(code); } catch (e) {
      throw new Error(`${name}: синтаксическая ошибка в ${rel} — ${e.message}`);
    }
  }
  try { new Function(bundle); } catch (e) {
    throw new Error(`${name}: собранная страница не разбирается — ${e.message}`);
  }
