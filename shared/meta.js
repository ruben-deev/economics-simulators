...
// Личные таблицы рекордов всех игр. Наследие НОВОГРАДА подтягивается и из
// них (не только из введённых строк) — поэтому «сбросить всё» обязан
// стирать и рекорды, иначе перенос из прошлой игры переживает сброс.
export const GAME_RECORDS_KEYS = [
  'novoeda-records', 'kinoreka-records', 'biletville-records', 'novograd-records',
];

/**
 * Полный сброс пути серии: строки наследия, лучший финал НОВОГРАДА,
 * незаконченные партии и личные таблицы рекордов всех четырёх игр, а также
 * запомненные места в мировой таблице. Устройство встречает серию «как в
 * первый раз». Две итерации починок: сначала сброс щадил сейвы (НОВОЕДА
 * встречала недоигранной партией), потом рекорды (НОВОГРАД продолжал
 * показывать перенос из прошлой игры — наследие читается и из рекордов).
 *
 * Не трогаются: мировая таблица на сервере (результаты уже отправлены),
 * имя игрока, язык и выбранный уровень сложности.
 */
export function resetEcosystemProgress() {
  const cleared = [];
  const fixed = [META_BEST_KEY, META_LINES_KEY, ...GAME_SAVE_KEYS, ...GAME_RECORDS_KEYS];
  try {
    // Запомненные места в мировой таблице: ключи с тегом игры и уровня
    for (let i = localStorage.length - 1; i >= 0; i--) {
      const k = localStorage.key(i);
      if (k && k.startsWith('lb-mine-')) fixed.push(k);
    }
