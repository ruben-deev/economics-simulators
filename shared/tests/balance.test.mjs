...
const BASE = {
  ...
  // Числа НОВОГРАДА пересняты после события «сооснователь за долю»: новое
  // событие занимает место в пуле, поэтому поток событий у всех опор сдвинулся
  НОВОГРАД: {
    'только хаб': [754e6, 0],
    'хаб и такси': [8.71e9, 0],
    экосистема: [9.31e9, 0],
  },
};
