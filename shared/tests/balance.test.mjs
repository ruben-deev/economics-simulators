...
const BASE = {
  ...
  // Числа НОВОГРАДА пересняты дважды: после события «сооснователь за долю»
  // (новое событие занимает место в пуле) и после правки цен в событиях
  // водителей и давления инвесторов
  НОВОГРАД: {
    'только хаб': [754e6, 0],
    'хаб и такси': [8.73e9, 0],
    экосистема: [9.29e9, 0],
  },
};
