...
const BASE = {
  ...
  КИНОРЕКА: {
    ...
    // Перезакреплено после сознательных правок баланса (аудит 2026-08):
    // нормализация прибыли при оценке, надбавка за жадный триал от самой
    // длины, возврат собственной библиотеки в оценку и починка блокбастера.
    // Опоры сошлись: разброс между ними был 2.9×, стал 1.9×. Последний
    // сдвиг — от выгорания кризисов: нерешённый кризис больше не висит до
    // конца партии, и опоры (они не решают кризисы вовсе) перестали платить
    // неограниченный штраф. Прежний уровень держался на смертельной спирали,
    // о которой игрок не мог рассуждать.
    лицензионная: [9.22e9, 0],
    ровная: [9.03e9, 0],
    студийная: [4.37e9, 0],
  },
