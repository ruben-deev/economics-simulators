...
const BASE = {
  ...
  КИНОРЕКА: {
    ...
    // Перезакреплено после двух сознательных правок баланса (аудит 2026-08):
    // нормализация прибыли при оценке и надбавка за жадный триал, считаемая
    // от самой длины, а не от отклонения от опорной точки.
    лицензионная: [5.09e9, 0],
    ровная: [4.82e9, 0],
    студийная: [1.73e9, 0],
  },
