...
/**
 * Изменение против прошлого хода: процент, а если считать его не от чего —
 * само изменение.
 *
 * Прошлый ход бывает нулевым: первая неделя без открытых районов, месяц без
 * единого организатора, партия, где выручки ещё не было. Процент от нуля не
 * значит ничего, а деление на почти-ноль однажды показало игроку «заказы
 * +13 058 959 124 886%». Поэтому при базе меньше единицы возвращаем разницу
 * в натуре: «+131» или «+2,3 млн ₽». Такая подстановка одинаково хорошо
 * читается и в скобках, и после предлога «на».
 *
 * fmt — как показать разницу, когда процента нет (числом или деньгами).
 */
export function growth(now, before, fmt, digits = 0) {
