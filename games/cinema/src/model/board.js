...
export function makeGoal(year, state, yourSubs, rivalSubs) {
  ...
  if (year === 2) {
    return {
      ...
      // порог качества (у случайных стратегий прибыльных месяцев ноль и на
      // 90-м процентиле), это сознательно самая жёсткая цель партии.
      target: 3,
  ...
  return {
    ...
    // Планка выставляется замером между медианой и 90-м процентилем.
    // После третьего акта (рывок конкурента) распределение сдвинулось вниз:
    // медиана доли 0.34, 90-й процентиль 0.62 — прежние 0.60 брали 11%.
    target: 0.55,
