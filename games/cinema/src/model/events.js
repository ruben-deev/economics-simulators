  ...
  // --- События с выбором ---
  {
    ...
    options: [
      ...
      },
      {
        ...
        secret: true,
        label: { ru: 'Заменить актёра — сгенерирует нейросеть «СКРЕПКА»', en: 'Recast the role — let the PAPERCLIP AI generate the actor' },
        detail: {
          ru: 'Экономим деньги, но зрители замечают и уходят. Чем дороже рынок таланта, тем чаще замена — правильный ответ. Гонораров СКРЕПКА не просит; правда, герой моргает строго раз в 4,7 секунды.',
          en: 'Saves the money, but viewers notice and leave. The pricier the talent market, the more often recasting is the right call. PAPERCLIP asks for no fee — though the hero blinks exactly once every 4.7 seconds.',
        },
  ...
  },
  {
    ...
    options: [
      ...
      },
      {
        ...
        // священна), различие чисто сюжетное.
        secret: true,
        label: { ru: 'Отстоять цену — объяснит нейросеть «СКРЕПКА»', en: 'Hold the price — let the PAPERCLIP AI explain' },
        detail: {
          ru: 'Оценка ниже: инвесторы хотели скидку и её не получили. СКРЕПКА вежливо объясняет совету, что скидка — это кредит у собственной выручки; совет недоволен, но впечатлён.',
          en: 'The valuation suffers: the investors wanted a discount and did not get one. PAPERCLIP politely explains to the board that a discount is a loan taken from your own revenue; they are unhappy, but impressed.',
        },
