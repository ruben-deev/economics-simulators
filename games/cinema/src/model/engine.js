...
export function step(prevState, input = {}) {
  ...
  for (const def of SEGMENTS) {
    ...
    // Надбавка считается от самой длины, а не от отклонения от опорной точки.
    // Раньше излом стоял ровно на четырнадцати днях, и весь участок ниже был
    // выигрышем без обратной стороны: замер на 120 кодах показывал 5.2 млрд
    // на семи днях против 11.1 на четырнадцати, причём ответ «четырнадцать»
    // не зависел ни от цены, ни от рекламы, ни от закупки. Рычаг с ответом,
    // одинаковым во всех партиях, — это не решение, а спрятанная константа.
    // Бесплатного месяца ради бесплатного месяца хотят и на коротком триале,
    // просто меньше.
    // Квадрат, а не прямая: на трёхдневном триале бесплатного месяца ради
    // бесплатного месяца почти никто не ищет, а тридцатидневный позволяет
    // досмотреть сезон целиком и уйти. Линейная надбавка облагала налогом
    // и короткий триал, роняя все опоры вдвое.
    const trialGreed = clamp((decisions.trialDays / 30) ** 2, 0, 1);
  ...
  // Пробники тоже смотрят — и трафик за них платите вы. Раньше этой строки не
  // было, и пробный период стоил только недополученной подписки: получался
  // бесплатный разгон, где рычаг просто выкручивался до опорных четырнадцати
  // дней в любой партии. Теперь длина триала упирается в главную переменную
  // статью бизнеса, и правильный ответ зависит от того, сколько у вас смотрят
  // и почём вам час.
  //
  // Считаем по тем, кто НЕ дошёл до оплаты: конвертировавшиеся уже в базе и
  // посчитаны выше. Смотрят они долю месяца, равную длине пробного периода.
  const trialViewers = Math.max(0, trialsTotal - newSubs);
  const trialHours = trialViewers * CONFIG.baseHoursPerSub
    * clamp(decisions.trialDays / 30, 0, 1) * season;
  hours += trialHours;
...
export function valuation(state) {
  ...
  const rawMargin = marginWindow > 0
    ? windowAvg(h, CONFIG.valuationWindow, (r) => r.profit) / marginWindow : -0.5;

  // Качество прибыли. Подготовить компанию к выходу — свернуть закупку и
  // маркетинг за полгода до сделки — реальная практика, и маржа от этого
  // действительно растёт. Но покупатель на это и смотрит: перед сделкой
  // заработок нормализуют к тому уровню вложений, при котором бизнес не
  // сжимается. Без нормализации замер показывал, что тишина с 30-го месяца
  // поднимает итог в 2.26 раза — то есть выгоднее было бросить бизнес, чем
  // строить его.
  //
  // Норма берётся по всей партии: тот, кто был скромен всегда, ничего не
  // теряет — вычитается только разрыв между привычным уровнем вложений и
  // последними месяцами. Доли от выручки, а не рубли, чтобы выросший бизнес
  // не штрафовался за то, что та же закупка стала меньшей долей.
  const invRate = (r) => (r.contentSpend + r.marketingSpend) / Math.max(1, r.revenue);
  const recentInv = windowAvg(h, CONFIG.valuationWindow, invRate);
  const normalInv = windowAvg(h, h.length, invRate);
  const underInvest = Math.max(0, normalInv - recentInv) * CONFIG.qualityOfEarnings;
  const margin = rawMargin - underInvest;
