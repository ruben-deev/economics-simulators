...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// Схема «Полка и конвейер» — главный орган КИНОРЕКИ.
//
// Отвечает на её собственный вопрос: чем наполнена полка и что выйдет
// дальше. Три вещи в одной картинке:
//   полка   — лицензии тают каждый месяц, оригиналы остаются навсегда;
//   конвейер— что снимается сейчас и когда выйдет;
//   календарь — ваши премьеры против премьер конкурента.
//
// Про календарь честно: вперёд вы знаете СВОЙ график (проекты в
// производстве и готовое в запасе), а про конкурента — только анонс на
// следующий месяц. Ровно так устроена и модель, и рынок.
// ----------------------------------------------------------------------------
function renderStudioMap() {
  ...
  const licensed = state.catalogLicensed ?? 0;
  const original = state.catalogOriginal ?? 0;
  const decay = licensed * CONFIG.licenseDecay;
  const fresh = catalogFreshness(state.freshHours ?? 0);
  const slate = state.slate ?? [];
  const producing = slate.filter((p) => p.status === 'production');
  const ready = slate.filter((p) => p.status === 'ready');

  // Полка: высота столбика — часы каталога. Опора шкалы растёт вместе с
  // библиотекой, иначе крупный каталог упирается в потолок и перестаёт расти.
  const maxHours = Math.max(800, licensed + original);
  const barH = (v) => Math.max(2, Math.round(110 * (v / maxHours)));
  const baseY = narrow ? 148 : 158;
  const shelfX = narrow ? 34 : 46;
  const barW = narrow ? 46 : 54;
  const gap = narrow ? 66 : 78;

  const bar = (x, value, color, label, sub) => `
    <rect x="${x}" y="${baseY - barH(value)}" width="${barW}" height="${barH(value)}"
      rx="3" fill="${color}" opacity="0.75"/>
    <text x="${x + barW / 2}" y="${baseY + 16}" text-anchor="middle" class="m-muted">${label}</text>
    <text x="${x + barW / 2}" y="${baseY - barH(value) - 6}" text-anchor="middle" class="m-num">${compact(value)}</text>
    ${sub ? `<text x="${x + barW / 2}" y="${baseY + 30}" text-anchor="middle" class="m-muted" style="fill:var(--bad)">${sub}</text>` : ''}`;

  // Календарь: несколько месяцев назад (что уже вышло) и вперёд (что выйдет).
  // На узком экране окно короче — читаемость важнее охвата.
  // Окно вперёд подстраивается под конвейер: у сезона пять-шесть месяцев
  // производства, и при коротком окне премьеры оказывались за краем карты —
  // то есть ровно то, ради чего календарь и рисуется, было не видно.
  const maxLeft = producing.reduce((a, p) => Math.max(a, p.monthsLeft), 0);
  const back = narrow ? 1 : 3;
  const fwd = Math.min(narrow ? 5 : 7, Math.max(narrow ? 3 : 4, maxLeft));
  const now = state.month;
  const months = [];
  for (let m = now - back; m <= now + fwd; m++) if (m >= 1) months.push(m);

  const tlX = narrow ? 18 : 250;
  const tlY = narrow ? 268 : 124;
  const slotW = narrow ? Math.floor(324 / months.length) : Math.floor(436 / months.length);

  // Свои премьеры: в прошлом — из отчётов, в будущем — из конвейера и запаса
  const ownAt = (m) => {
    if (m <= now) return (h.find((x) => x.month === m)?.premieres ?? 0);
    return producing.filter((p) => now + p.monthsLeft === m).length;
  };
  const rivalAt = (m) => {
    if (m < now) return h.find((x) => x.month === m)?.rival ?? 'none';
    if (m === now) return r?.rival ?? 'none';
    if (m === now + 1) return state.rivalNext ?? 'none';
    return null;   // дальше следующего месяца конкурент не анонсирует
  };

  const cells = months.map((m, i) => {
    const x = tlX + i * slotW + slotW / 2;
    const own = ownAt(m);
    const riv = rivalAt(m);
    const rivIcon = riv === null ? '?' : (RIVAL_RELEASES[riv]?.icon ?? '·');
    const future = m > now;
    return `
      <text x="${x}" y="${tlY - 26}" text-anchor="middle" class="${own > 0 ? 'm-num' : 'm-muted'}"
        style="fill:${own > 0 ? PALETTE[1] : 'var(--muted)'}">${own > 0 ? `🎬${own > 1 ? `×${own}` : ''}` : '·'}</text>
      ${m === now && ready.length ? `<text x="${x}" y="${tlY - 44}" text-anchor="middle" class="m-muted"
        style="fill:${PALETTE[4]}">📦${ready.length}</text>` : ''}
      <rect x="${tlX + i * slotW + 2}" y="${tlY - 12}" width="${slotW - 4}" height="24" rx="4"
        fill="${m === now ? 'rgba(96,165,250,0.16)' : 'transparent'}"
        stroke="${m === now ? PALETTE[1] : 'var(--line)'}" stroke-width="1"
        stroke-dasharray="${future ? '4 3' : '0'}"/>
      <text x="${x}" y="${tlY + 5}" text-anchor="middle" class="m-muted">${m}</text>
      <text x="${x}" y="${tlY + 34}" text-anchor="middle" class="m-muted"
  ...
  box.innerHTML = `<div class="panel eco-map">
    ...
    <svg viewBox="${viewBox}" class="${narrow ? 'map-v' : ''}" role="img" aria-label="${t('studioMapTitle')}">
      <text x="${narrow ? 12 : 14}" y="${baseY - 124}" class="m-muted">${t('studioShelf')}</text>
      <line x1="${shelfX - 12}" y1="${baseY + 1}" x2="${shelfX + gap + barW + 12}" y2="${baseY + 1}"
        stroke="var(--line)" stroke-width="1"/>
      ${bar(shelfX, licensed, PALETTE[3], t('studioLicensed'), decay > 1 ? `−${compact(decay)} ${t('studioPerMonth')}` : '')}
      ${bar(shelfX + gap, original, PALETTE[1], t('studioOriginal'), '')}
