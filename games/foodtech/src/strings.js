...
export const STRINGS = {
  ...
  mapLegend: {
    ru: 'Квартал — район. Размер — сколько в нём жителей, цвет — вклад с заказа (зелёный приносит, красный съедает), домики — подключённые рестораны (один домик на тридцать), ниточка — плечо доставки от ресторана к клиенту того же района (красная значит, что район не укладывается в эталонные минуты). Пунктир говорит про конец недели: зелёный откроется, красный закроется, серый закрыт и ждёт входа. Нажмите на квартал, чтобы открыть район или убрать его из плана.',
    en: 'A block is a district. Its size is how many people live there, its colour is contribution per order (green earns, red eats), the little houses are restaurants signed up (one house per thirty), and the thread is the delivery leg from a restaurant to a customer in the same district (red means the district misses the reference time). A dashed outline is about the end of the week: green opens, red closes, grey is closed and waiting for its entry fee. Click a block to open the district or drop it from the plan.',
  },
...
score = valuation × your stake`,
  ...
  gradeScale: { ru: 'Шкала: «отлично» — итог от {a}, «крепкий бизнес» — от {b}, «выжили» — от {c}. Доведённые стратегии на этом рынке заканчивают партию с итогом около 9 млрд ₽ — выжить легко, выиграть нет.', en: 'The scale: "excellent" starts at {a}, "a solid business" at {b}, "survived" at {c}. Polished strategies finish this market above $90M — surviving is easy, winning is not.' },
