...
score = valuation × your stake`,
  ...
  welcomeTension: { ru: '<b>В чём подвох.</b> Человек платит за доставку 149 ₽, а курьер за ту же поездку получает 180 ₽ — каждый заказ в минусе. Поднимете цену — закажут реже; урежете курьерам — уйдут, и еда поедет дольше. Выход есть, но не в одном ползунке.', en: '<b>Where the catch is.</b> A customer pays ₽149 for delivery while the courier gets ₽180 for the same trip — every order loses money. Raise the price and people order less; cut courier pay and couriers leave, so food arrives later. There is a way out, but it is not one slider.' },
