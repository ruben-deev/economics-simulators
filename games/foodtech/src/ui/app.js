...
function otherGamesHtml() {
  ...
  return `<div style="margin-top:8px">
    ...
    <div style="display:flex;gap:8px">
      <a class="btn small primary" style="flex:1;text-align:center" href="../cinema/index.html">${t('tryOthersA')}</a>
...
function showWelcome() {
  ...
  if (seedWanted === null) seedWanted = urlGameCode() || weeklySeedToPlay('НОВОЕДА');
  ...
  const startGame = () => {
    ...
    if ((v && v !== state.seed)) {
      state = createInitialState(v || state.seed);
      save();
      renderAll();
    }
  };
