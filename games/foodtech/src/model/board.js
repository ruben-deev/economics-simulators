...
export function makeGoal(quarter, state, orders, customers) {
  ...
  if (quarter === 2) {
    return {
      ...
      // Снижено с 60 после ужесточения погоды: шторма теперь едят вклад,
      // и прежняя планка проходила в 19% вместо целевых ~30%.
      target: 50,
  ...
  if (quarter === 3) {
    return {
      ...
      // отделяет работающий бизнес от почти работающего, а не наказывает
      // за неудачный месяц. Снижено с 6 до 4 после ужесточения погоды: шторм
      // теперь способен съесть прибыль недели даже у собранной стратегии.
      // Калибровка по случайным стратегиям здесь не работает: прибыльность —
      // порог качества, у случайных стратегий прибыльных недель ноль и на
      // 85-м процентиле, и планка 2 проходит так же редко, как 6 (замер: 8%).
      // Это сознательно самая жёсткая цель года: доведённая стратегия
      // закрывает в плюс все тринадцать недель.
      target: 4,
      // И не ценой базы: срезать расходы, растеряв клиентов, — не то,
      // за что хвалят.
      customersFloor: Math.round(Math.max(20_000, customers * 0.85)),
      reward: 0.12,
      penalty: 'marketingCap',
    };
  }
