    ...
    options: [
      {
        ...
        secret: true,
        label: { ru: 'Доплатить (2 500 ₽ на курьера) — переведёт нейросеть «СКРЕПКА»', en: 'Pay up ($25 per courier) — let the PAPERCLIP AI send it' },
        detail: {
          ru: 'Цена зависит от штата: сотне курьеров это четверть миллиона, полутора тысячам — почти четыре. СКРЕПКА сутки читает курьерский чат, проникается и переводит каждому с пометкой «держитесь там».',
          en: 'The price scales with the fleet: $2,500 for a hundred couriers, nearly $40K for fifteen hundred. PAPERCLIP reads the courier chat for a day, is moved, and wires each of them with the note “hang in there”.',
        },
  ...
  },
  {
    ...
    options: [
      ...
      },
      {
        ...
        // священна), различие чисто сюжетное.
        secret: true,
        label: { ru: 'Отстоять юнит-экономику — доложит нейросеть «СКРЕПКА»', en: 'Defend the unit economics — let the PAPERCLIP AI present' },
        detail: {
          ru: 'Инвесторы недовольны, оценка ниже. СКРЕПКА готовит совету 4 000 слайдов о юнит-экономике; совет сдаётся на 812-м. Принципы целы.',
          en: 'Investors are unhappy and the valuation suffers. PAPERCLIP prepares 4,000 slides on unit economics for the board; they surrender at slide 812. Principles intact.',
        },
...
export function rollEvent(rng, week, flags = {}, seen = []) {
  if (week < 2) return null;
  if (rng() > 0.35) return null;
  let pool = EVENTS.filter((e) => week >= (e.minWeek ?? 0));
  if (flags.regulationRisk) pool.push(REGULATION_FINE);
  pool = boostCarriers(pool, seen, week >= 40);
