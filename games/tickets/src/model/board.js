...
export function makeGoal(year, state, yourOrgs, rivalOrgs) {
  if (year === 1) {
    return {
      year,
      type: GOAL_TYPES.gmv,
      // Оборот за месяц к концу первого года. Планка опущена с 520 млн после
      // того, как переезд организаторов на виджет перестал быть мгновенным
      // и бесплатным: замер по девяти десяткам стратегий даёт медиану 326 млн
      // и 75-й процентиль 421 млн, так что прежние 520 млн брали единицы.
      target: 380_000_000,
      reward: 0.10,
      penalty: 'dilution',
    };
