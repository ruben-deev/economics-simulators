// ============================================================================
// Движок билетного сервиса «БИЛЕТОН».
//
...
// Начальное состояние
// ----------------------------------------------------------------------------
export function createInitialState(seed = 'bileton') {
  ...
  const state = {
    ...
    // Кому уже открыта своя касса. В начале — никому: платформы ещё нет.
...
export function step(prevState, input = {}) {
  ...
  // --- 4. Организаторы ---
  // Решение о собственной кассе принимается по типам. Отключение уже
  ...
  // через вас. Всё, что организатор продаёт своей кассой, в вашей афише не
  // появляется — и работать на ваш охват не может. Именно поэтому раздача
  // собственных касс всем подряд подрезает ту самую аудиторию, ради которой
