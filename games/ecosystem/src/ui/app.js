...
// Город Новоград
//
// Первая версия рисовала круги Венна: базы сервисов и их пересечение. Схема
// была верной, но пустой первые десять месяцев — один круг и два призрака с
// подписью «такси не запущено», — и читалась диаграммой, а не городом.
//
// Здесь то же самое нарисовано городом: сверху сервисы зданиями с потоками
// месяца, снизу горожане. Значок — фиксированное число взрослых, и толпа не
// меняется в размере всю партию: меняется её цвет. Серый горожанин не ваш,
// синий пользуется одним вашим сервисом, золотой — двумя и более. Золото и
// есть то, за что инвестор платит премию за склейку, поэтому его видно
// глазом, а не только строкой в оценке.
// ----------------------------------------------------------------------------
const CROWD_UNITS = [500, 1e3, 2e3, 5e3, 1e4, 2e4, 5e4];
...
function renderEcoMap() {
  ...
  const multi = r ? (r.multiUsers ?? 0) : 0;
  const taxiOn = state.taxi.on;
  const ecomOn = state.ecom.on;
  ...
  const perRow = narrow ? 13 : 26;
  const rowsMax = narrow ? 12 : 6;
  const unit = CROWD_UNITS.find((u) => CONFIG.cityAdults / u <= perRow * rowsMax)
    ?? CROWD_UNITS[CROWD_UNITS.length - 1];
  const dots = Math.round(CONFIG.cityAdults / unit);
  // Клиент двух сервисов занимает один значок, а не два: в толпе стоят люди,
  // а не подписки, поэтому шкала — уникальные клиенты
  const goldDots = Math.min(dots, Math.round(multi / unit));
  const blueDots = Math.min(dots - goldDots, Math.round(Math.max(0, unique - multi) / unit));

  const sw = narrow ? 26 : 25;
  const sh = narrow ? 26 : 26;
  const headY = narrow ? 126 : 120;
  const x0 = (W - perRow * sw) / 2 + sw / 2;
  const rows = Math.ceil(dots / perRow);
  const H = headY + rows * sh + 12;

  // Горожанин: голова и плечи. Мельче уже не человек, крупнее не помещается
  // город целиком, а он должен читаться одним взглядом.
  const person = (i) => {
    const x = x0 + (i % perRow) * sw;
    const y = headY + Math.floor(i / perRow) * sh;
    const cls = i < goldDots ? 'p-multi' : (i < goldDots + blueDots ? 'p-one' : 'p-none');
    return `<g class="c-person ${cls}">
      <circle cx="${x}" cy="${y}" r="3.4"></circle>
      <path d="M ${x - 5.4} ${y + 12} C ${x - 5.4} ${y + 4.4}, ${x + 5.4} ${y + 4.4}, ${x + 5.4} ${y + 12} Z"></path>
    </g>`;
  };

  // Здание сервиса: тёмное и пунктирное, пока сервис не запущен. Клик по нему
  // ведёт к рычагам этого сервиса — как и раньше у кругов.
  const house = (i, on, icon, name, users, jump, offLabel, inflow, outflow, color) => {
    const bw = narrow ? 104 : 200;
    const bh = narrow ? 54 : 62;
    const bx = narrow ? 8 + i * (bw + 12) : 24 + i * (bw + 26);
    const by = narrow ? 30 : 22;
    return `<g class="c-house${on ? ' on' : ''}" data-jump="${on ? jump : 'panel:verticals'}"
      style="--house:${color}">
      <path d="M ${bx} ${by + 14} L ${bx + bw / 2} ${by} L ${bx + bw} ${by + 14} Z"></path>
      <rect x="${bx + 6}" y="${by + 14}" width="${bw - 12}" height="${bh - 14}" rx="3"
        ${on ? '' : 'stroke-dasharray="5 4"'}></rect>
      <text x="${bx + bw / 2}" y="${by + 30}" text-anchor="middle" class="m-name">${icon} ${
        on ? name : offLabel}</text>
      ${on ? `<text x="${bx + bw / 2}" y="${by + 46}" text-anchor="middle" class="m-num">${
        compact(users)}</text>` : ''}
      ${!narrow && on && (inflow > 0.5 || outflow > 0.5) ? `<text x="${bx + bw / 2}" y="${by + bh + 12}"
        text-anchor="middle" class="m-muted">${inflow > 0.5 ? `<tspan class="pos">+${
        compact(inflow)}</tspan>` : ''}${inflow > 0.5 && outflow > 0.5 ? ' · ' : ''}${
        outflow > 0.5 ? `<tspan class="neg">−${compact(outflow)}</tspan>` : ''}</text>` : ''}
    </g>`;
  };

  const badges = [];
  if (r && r.warMonthsLeft > 0) badges.push(`⚔️ ${tx(taxi.incumbentName)} · ${r.warMonthsLeft}`);
  if (r && r.fedMonthsLeft > 0) badges.push(`🏴 ${t('mapFed', { months: r.fedMonthsLeft })}`);
  if (r && r.crisisMonthsLeft > 0) badges.push(`📉 ${t('mapCrisis', { months: r.crisisMonthsLeft })}`);

  const dotsMarkup = Array.from({ length: dots }, (_, i) => person(i)).join('');

  box.innerHTML = `<div class="panel eco-map crowd">
    <h2 class="panel-title">${t('mapTitle')}</h2>
    <svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${t('mapTitle')}">
      <text x="14" y="${narrow ? 18 : 14}" class="m-muted">${t('mapCity', {
        adults: compact(CONFIG.cityAdults) })}</text>
      ${house(0, true, asset.icon, t('mapHub'), foodU, 'group:food', '',
        r ? (r.wonBack ?? 0) + (r.organicFood ?? 0) : 0, r ? (r.lostFood ?? 0) : 0, PALETTE[1])}
      ${house(1, taxiOn, taxi.icon, t('mapTaxi'), taxiU, 'group:taxi', t('mapTaxiOff'),
        r ? (r.coldAcq ?? 0) + (r.crossConv ?? 0) : 0, r ? (r.lostTaxi ?? 0) : 0, PALETTE[2])}
      ${house(2, ecomOn, ecomDef.icon, t('mapEcom'), ecomU, 'group:ecom', t('mapEcomOff'),
        r ? (r.ecomColdAcq ?? 0) + (r.crossEcomConv ?? 0) : 0, r ? (r.lostEcom ?? 0) : 0, PALETTE[4])}
      <line x1="10" y1="${headY - 16}" x2="${W - 10}" y2="${headY - 16}" class="c-street"></line>
      ${dotsMarkup}
    </svg>
    <div class="flow-legend">
      <span class="flow-key"><i style="background:${PALETTE[3]}"></i>${t('mapMulti', {
        n: compact(multi) })}</span>
      <span class="flow-key"><i style="background:${PALETTE[1]}"></i>${t('mapOne', {
        n: compact(Math.max(0, unique - multi)) })}</span>
      <span class="flow-key"><i class="k-none"></i>${t('mapNotYours')}</span>
      ${plusOn ? `<span class="flow-key">➕ ${
        t('mapPlus', { subs: compact(state.plus.subs) })}</span>` : ''}
      ${badges.map((b) => `<span class="flow-key">${b}</span>`).join('')}
    </div>
    <div class="chart-caption">${t('mapUnique', {
      unique: compact(unique), share: pct(unique / CONFIG.cityAdults, 0),
      person: compact(unit),
    })}${narrow ? '' : ` ${t('mapFlows')}`}</div>
  </div>`;
}
