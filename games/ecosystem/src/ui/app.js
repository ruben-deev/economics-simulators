...
// Тег партии в строке результата и в мировой таблице.
// Ступени сложности больше нет: taggedGame возвращает базовый тег, и старые
// записи с суффиксом попадают в ту же таблицу. Обёртка оставлена, чтобы не
// разносить одно и то же изменение по трём местам вызова.
...
// не читается — см. load(). Поднято при снятии рычага «финансовая команда»
// (аудит 2026-08): игрок за неё больше не платит, а ставка «прочих расходов»
// зафиксирована — опоры сдвинулись на 1.5–11%. Партия, начатая на прежней
// экономике, доигрывалась бы по новой: и график, и запись в таблицу
// рекордов получились бы из двух разных игр.
const BUILD = 'ecosystem-3';
...
function syncLevers() {
  for (const l of LEVERS) {
    ...
    if (l.policy) {
      ...
      if (val) val.textContent = `${tx(current.label)} · ${+raw.toFixed(2)}%`;
...
function showWelcome() {
  ...
  if (seedWanted === null) seedWanted = urlGameCode() || weeklySeedToPlay('НОВОГРАД');
...
function showHelp(fromWelcome = false) {
  ...
  modal(`<h2>${t('helpModalTitle')}</h2>${renderHelpTab()}`
    + (fresh ? seedFieldHtml() : `<p class="funding-note">${t('helpSeed', { seed: state.seed })}</p>`)
