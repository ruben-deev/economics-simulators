...
export const CONFIG = {
  ...
  // не двигает спрос и НЕ ЗАБИРАЕТ СЕРВИС — все вертикали и все рычаги
  // остаются у игрока. Он тянет управленческую нагрузку объёмом в целую
  // вертикаль, то есть снимает ровно один слот конгломератного штрафа
  // (focusSlot), а не его долю. Первая формулировка была «забирает одну
  // вертикаль под себя» — читалось так, будто у игрока отнимают бизнес, и
  // непонятно какой; в модели ничего подобного нет, штраф симметричен.
  //
  ...
  // --- Инвестиции ---
  // Окно раундов открывается со второго месяца, а не с третьего. Двойка
  // создавала ловушку: билетный старт (касса 160 млн против 200–220 у
  // остальных, запуск такси стоит столько же) разорялся на втором месяце —
  // за месяц до того, как появлялась хоть какая-то возможность занять.
  // Разница между 1.5 млрд и банкротством была шесть миллионов в месяц.
  // Замер (24 кода): «жирная» политика на билетах 45 млн и 24 провала из 24
  // при окне с третьего месяца, 1.45 млрд и ноль провалов — со второго;
  // потолок при этом не сдвинулся («скупая» политика 1.48 против 1.48).
  // Записанный коридор меряется на доставке и уехал на 0.3% (8.00 -> 7.97,
  // 8.71 -> 8.69) — это шум, а не сдвиг баланса, поэтому не переснимался.
  minMonthForFunding: 1,
  ...
  // «Парк как капитал»: деньги превращаются в железо, железо стареет только
  // на улице и продаётся дешевле, чем куплено. Единственная вертикаль, где
  // главный рычаг — не бюджет, а календарь: план года улица/склад против
  // самой жёсткой сезонности в наборе (зима ×0.1, июль ×1.7).
  // Открывается автоматически с годом конгломерата; пока парк не куплен,
  // не стоит ни рубля и не размывает фокус.
  // Числа замерены (12 сидов, база — якорь «экосистема», медианы):
  // грамотный парк 26 партий с планом «улица март–октябрь» и усиленной
  // управляющей компанией даёт +1.5 п.п. роста года конгломерата; тот же
  // парк «круглый год» — хуже, чем совсем без самокатов (−1.1 п.п. и лишний
  // риск кассы); парк без усиления управляющей компании теряет −1.2 п.п.
  // на расфокусе всего холдинга. План есть за что считать: улица ровно в
  // восемь месяцев жизни парка, зима — на складе.
  scooters: {
    batchUnits: 100,          // самокаты покупаются партиями
    unitCost: 40_000,         // ₽ за самокат
    streetLifeMonths: 8,      // столько уличных месяцев живёт самокат
    resaleShare: 0.5,         // цена продажи = остаток жизни x эта доля цены
    maintenancePerUnit: 2_400, // ₽/мес на улице: зарядка, ремонт, перестановка
    storagePerUnit: 400,       // ₽/мес на складе: аренда и консервация
    fixedMonthly: 1_000_000,   // операционный штаб — только пока парк существует
    ridesPerUnitMonth: 115,    // ёмкость: поездок с самоката в месяц
    ridePrice: 130,            // ₽ за поездку — базовый тариф (priceRef)
    rideVarCost: 30,           // ₽ себестоимости поездки
    // Тариф поездки — режимный рычаг: спрос отвечает по эластичности от
    // базового тарифа. Логика решения: когда спрос упирается в парк (лето,
    // малый парк) — дорогой тариф снимает сливки с очереди; в межсезонье
    // база и дешёвый добирают поездки, дешёвый ещё и разгоняет райдеров.
    // Замер (24 сида, парк 26 партий, улица март–октябрь, чистый денежный
    // итог вертикали за год): помесячный выбор тарифа 98.5 млн, «всегда
    // 165» 86.5, «всегда 130» 56.1, «всегда 99» −4.5 — правильный тариф
    // месяца даёт +14% к лучшей константе, а «поставил и забыл» на базе
    // теряет 43%. Асимметрия эластичности (2.2 вверх против 1.6 вниз)
    // выбрана именно ради этой вилки: без неё «всегда дорого» доминировало.
    priceModes: [99, 130, 165],
    // Асимметрия: подорожание гонит райдеров на такси и пешком быстрее,
    // чем удешевление приводит новых — вверх спрос отвечает круче
    priceElasticity: 1.6,
    priceElasticityUp: 2.2,
    // Дешёвый тариф — ещё и маркетинг: райдеры прирастают быстрее
    // (и монетизируются склейкой через Plus), дорогой — медленнее
    adoptElasticity: 1.0,
    cityDemandRides: 380_000,  // спрос города в месяц при сезоне 1.0
    // Сезон по календарю, январь..декабрь. Год конгломерата начинается
    // в январе — первым же ходом парк на улице теряет деньги.
    season: [0.05, 0.1, 0.4, 0.9, 1.2, 1.6, 1.7, 1.6, 1.1, 0.7, 0.25, 0.05],
    ridersPerUnit: 20,         // райдеров держит один уличный самокат
    riderAdopt: 0.5,           // скорость набора райдеров к ёмкости парка
    riderDecay: 0.1,           // отток райдеров за месяц без самокатов на улице
    hubOverlap: 0.7,           // доля райдеров, уже знакомых хабу (склейка)
    plusOverlapBoost: 0.08,    // подписка Plus открывает самокаты бесплатно
    maxBatchesPerMonth: 10,    // физика закупок: больше за месяц не привезут
    focusWeight: 0.5,          // полвертикали в штрафе фокуса: склад и бригада, а не второе такси
    // Зимняя улица изнашивает вдвое: реагенты, соль и морозы убивают батареи.
    // Порог по сезону: месяцы со спросом ниже winterSeasonMax считаются зимой.
    winterWearMult: 2,
    winterSeasonMax: 0.3,
  },

  // --- Перенос финала игры-источника ---
  // Из прошлой игры в НОВОГРАД переходят три валюты финала: КЛИЕНТЫ (база,
  // которую вы вырастили), КАССА (деньги, которые вы там не потратили) и
  // ОЦЕНКА (репутация у инвесторов). Масштаб — отношение вашего счёта к
  // «крепкому» порогу той игры: дескриптор стартового актива и есть крепкий
  // финал (1.0), всё сверх него приходит числами. Сверху срезано: правило
  // набора — заслуженный вход даёт преимущество, а не победу. Размер замерен.
  legacyCarry: {
    // Клиенты: база и пул возврата растут вместе. Планки правления считаются
    // от ПЕРЕНЕСЁННОЙ базы, а не от дескриптора: большая компания обязана
    // и вторую ногу построить большую — иначе перенос обходил бы цели.
    // Потолок переноса — финал вдвое крупнее крепкого (LEGACY_RATIO_CAP = 2),
    // то есть доведённая партия игры-источника. Срез по силе переноса
    // клиентов (8 сидов, медиана к «только флаг»): +6% базы даёт +2.5%
    // итога, +12% — +10.3%, +21% — уже +33%: база нелинейна, лишний клиент
    // тянет за собой кросс-селл и мультипликатор оценки. Взято +12% на
    // потолке — заметно, но партию не решает.
    usersPerRatio: 0.12,  // столько базы прибавляет финал вдвое крупнее крепкого
    usersCap: 0.12,
    cashPerRatio: 0.18,   // и столько казны (+2.5% итога)
    cashCap: 0.18,
    ...
    // Оценка прошлой компании — репутация у инвесторов: раунд оценивают
    // выше и не роняют ниже пола. Вместе +3.3% итога на потолке.
    roundPerRatio: 0.15,
    roundCap: 0.15,
    floorPerRatio: 0.30,
    floorCap: 0.30,
...
export const START_ASSETS = [
  {
    ...
    // Уровень сложности помогает активам неодинаково, и это замерено
    // (24 кода, одна политика): лёгкий поднимает этот старт в 1.38 раза,
    // сложный опускает в 0.97. Планка вердикта двигается вместе с ним —
    // иначе на лёгком «отлично» доставалось бы одному старту втрое чаще.
    gradeLevel: { easy: 1.38, hard: 0.97 },
  ...
  },
  {
    ...
    // Уровень сложности помогает активам неодинаково, и это замерено
    // (24 кода, одна политика): лёгкий поднимает этот старт в 1.34 раза,
    // сложный опускает в 0.92. Планка вердикта двигается вместе с ним —
    // иначе на лёгком «отлично» доставалось бы одному старту втрое чаще.
    gradeLevel: { easy: 1.34, hard: 0.92 },
  ...
  },
  {
    id: 'tickets',
    icon: '🎟️',
    fromGame: { ru: 'БИЛЕТВИЛЬ', en: 'TICKETGRAD' },
    name: { ru: 'Билетный сервис «Билетвиль»', en: 'Ticketgrad ticketing' },
    short: { ru: 'билеты', en: 'ticketing' },
    hint: {
      ru: 'Вы выиграли рынок билетов. Самая широкая база города и самая тонкая выручка с человека: билет покупают пару раз в год, зато сервис знает почти весь Новоград. Плюс партнёрская сеть организаторов, через которую дешевеет любое привлечение.',
      en: 'You won the ticketing market. The widest customer base in the city and the thinnest revenue per person: people buy a ticket twice a year, but almost all of Novograd knows the service. Plus a partner network of organisers that makes any acquisition cheaper.',
    },
    // Дескриптор пересобран (замечание владельца: «а почему у него самая
    // маленькая база и казна?»). Ответ оказался неприятным: ни из чего.
    // У двух других активов числа примерно отслеживают игру-источник —
    // победитель НОВОЕДЫ заканчивает со 131 тыс. клиентов при дескрипторе
    // 210 тыс. А у БИЛЕТВИЛЯ поля «база клиентов» нет вовсе: он считает
    // организаторов (у победителя их 195) и проданные билеты. Переносить
    // было нечего, число назначили маленьким, и под него написали легенду.
    //
    // При этом победитель БИЛЕТВИЛЯ продаёт 731 тыс. билетов в месяц при
    // охвате 12.7 млн и заканчивает партию с кассой 846 млн — самой полной
    // из трёх. Природа билетного бизнеса не «мало клиентов», а «много
    // клиентов, которые платят редко и мало»: билет покупают пару раз в год.
    // Прежний дескриптор делал актив маленьким дважды — и по базе, и по
    // чеку, — отсюда тянулись слабый кросс-селл (его ёмкость упирается в
    // размер базы), запертый третий сервис и вдвое более дорогое размытие.
    //
    // Теперь актив самый широкий по базе и самый тонкий по выручке с
    // клиента: 280 тыс. × 90 ₽ = 25 млн/мес против 61 у доставки и 50 у
    // стриминга. Замер на 24 кодах, лучшая тактика: было 2.35 млрд, стало
    // 6.72 при доставочных 11.33 — разрыв с 4.8 раза сжался до 1.7. Е-ком
    // теперь открывается на всех кодах (было 10 из 24), доля на финале
    // 48% вместо 37%.
    users: 280_000,
    arpu: 90,
    margin: 0.42,
    ...
    baseChurn: 0.022,
    returnPool: 41_000,
    reachableCap: 335_000,
    startCash: 200_000_000,
    // Пороги пересняты вместе с дескриптором: доли от замеренного потолка
    // те же, что у доставки (0.84 / 0.49 / 0.16 от лучшей тактики).
    grades: { excellent: 5.6e9, solid: 3.3e9, survived: 1.1e9, worthy: 3.3e9 },
    // Уровень сложности помогает активам неодинаково, и это замерено
    // (24 кода, одна политика): лёгкий поднимает этот старт в 1.04 раза,
    // сложный опускает в 0.94. Планка вердикта двигается вместе с ним —
    // иначе на лёгком «отлично» доставалось бы одному старту втрое чаще.
    gradeLevel: { easy: 1.04, hard: 0.94 },
...
/**
 * Шкала вердиктов для конкретной партии: своя у каждого актива и своя у
 * каждого уровня сложности. Второе не косметика: замер показал, что лёгкий
 * уровень поднимает крупные старты в 1.34–1.38 раза, а билетный — только в
 * 1.04, потому что экономия на прочих расходах компаундится через
 * мультипликатор оценки, а тонкий старт вдобавок сильнее размыт (48% доли
 * на финале против 74% у доставки).
 * Без поправки на уровень «отлично» на лёгком доставалось бы неравномерно.
 */
export function gradesFor(assetId, difficulty = 'normal') {
  const asset = assetById(assetId);
  const mult = asset.gradeLevel?.[difficulty] ?? 1;
  if (mult === 1) return asset.grades;
  return Object.fromEntries(
    Object.entries(asset.grades).map(([key, value]) => [key, Math.round(value * mult)]),
  );
}
...
// Витрина будущих фаз: в основной партии карточка заперта — дисциплина
// скоупа. Открывается вместе с годом конгломерата (см. CONFIG.scooters).
...
export const LEVERS = [
  ...
  },
  {
    ...
    policy: [
      ...
      { v: 399, label: { ru: '399 ₽ — премиум', en: '$3.99 — premium' },
