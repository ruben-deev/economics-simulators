...
export const EVENTS = [
  ...
  },
  {
    ...
    //
    // К аудиту доминации, перемерено 2026-08: отказ побеждает 72/72. Прежняя
    // запись здесь говорила про «живой выбор на полной экосистеме, 25/36» —
    // это больше не так, и, судя по замеру, не было так и раньше. Прямой
    // замер разницы «взять vs отказать» по уровням управляющей компании даёт
    // одну и ту же картину до и после снятия рычага финансовой службы:
    //
    //   УК 0 → −5%,  2 млн → −9%,  4 млн → −10%,  8 млн → −11%,  16 млн → −12%
    //
    // Сооснователь проигрывает ВЕЗДЕ, и медианы совпадают с точностью до
    // 0.1 п.п. с замером на прежней экономике: те 25/36 были хвостом кодов,
    // а не решением. Причина структурная: доля стоит 14% итога навсегда,
    // а снимает он четверть остатка штрафа фокуса — того самого, который
    // управляющая компания выкупает за 8 млн/мес, то есть за 288 млн против
    // 1.4 млрд. Одно и то же лечится двумя средствами разной цены.
    // Чинится это не числом, а развилкой (сооснователь должен уметь то,
    // чего не купить деньгами) — правка не сделана сознательно, чтобы не
    // менять баланс НОВОГРАДА заодно с починкой интерфейса.
    options: [
      {
