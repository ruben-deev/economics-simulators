...
export function makeGoal(year, state, uniqueUsers) {
  if (year === 1) {
    return {
      ...
      // Клиентов такси к 12-му месяцу. Замер по доведённым опорам:
      // средняя даёт ~76 тыс, размашистая ~89 тыс, бережливая с запуском
      // на 10-м месяце — ~27 тыс и цель проваливает: «вторая нога» требует
      // решения в первый год, и совет наказывает выжидание размытием.
      target: 40_000,
      reward: 0.10,
      penalty: 'dilution',
    };
