...
test('стартовые активы — данными: у стриминга свой контент, у билетов партнёрство', () => {
  ...
  // Прежде здесь стояло «у билетов казна меньше — сложный класс». Это
  // утверждение ни из чего не выводилось: победитель БИЛЕТВИЛЯ заканчивает
  // партию с самой полной кассой из трёх игр. Класс теперь отличается не
  // бедностью, а формой — широкая база при тонкой выручке (см. отдельный
  // тест), поэтому проверяем именно её, а не размер казны.
  assert.ok(tick.food.users > stream.food.users, 'база билетов должна быть шире стриминговой');
});
...
test('сооснователь забирает вертикаль: слот штрафа, а не его доля', () => {
  ...
  // Слот, а не доля: вертикалей две, партнёр ведёт одну — штрафить не за что.
  // Именно этим он отличается от управляющей компании, которая режет ПРОЦЕНТ:
  // долю штрафа можно докупить деньгами, слот — нельзя.
  assert.equal(after, 0, 'при двух вертикалях партнёр обязан снять штраф целиком');
  const three = { ...s, taxi: { ...s.taxi, on: true }, ecom: { ...s.ecom, on: true } };
  const withPartner = focusPenalty(three, { ...DEFAULT_DECISIONS, mgmt: 2_000_000 });
  const without = focusPenalty({ ...three, flags: { ...three.flags, cofounder: false } },
    { ...DEFAULT_DECISIONS, mgmt: 2_000_000 });
  assert.ok(Math.abs(without - withPartner * 2) < 1e-9,
    'при трёх вертикалях остаётся ровно половина штрафа');
...
// Замысел билетного старта — «широкий и тонкий»: самая большая база города
// при самой маленькой выручке с клиента. Раньше он был маленьким дважды, и
// это тянуло за собой слабый кросс-селл, запертый третий сервис и вдвое
// более дорогое размытие. Тест сторожит именно форму, а не конкретные числа.
test('билетный старт остаётся широким и тонким', () => {
  const tickets = assetById('tickets');
  const others = ['delivery', 'streaming'].map(assetById);
  for (const o of others) {
    assert.ok(tickets.users > o.users, `база билетов не шире, чем у ${o.id}`);
    assert.ok(tickets.arpu < o.arpu, `выручка с клиента у билетов не тоньше, чем у ${o.id}`);
    assert.ok(tickets.users * tickets.arpu < o.users * o.arpu,
      `выручка билетного актива не самая маленькая против ${o.id}`);
  }
  // Пороги вердикта считаются от замеренного потолка теми же долями, что у
  // доставки: иначе «отлично» раздавалось бы одному старту даром
  const d = assetById('delivery');
  const ratio = (a) => a.grades.excellent / a.grades.solid;
  assert.ok(Math.abs(ratio(tickets) - ratio(d)) < 0.25,
    'форма шкалы билетов разошлась с доставочной');
});
